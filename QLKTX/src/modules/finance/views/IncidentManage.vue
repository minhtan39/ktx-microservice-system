        ...
        <v-card-text class="incident-detail">
          ...
          <div class="detail-grid">
            <section class="action-panel">
              ...
              <v-select v-model="incidentForm.assignedTo" :items="staffOptions" item-title="title" item-value="value" label="Nhân viên phụ trách" variant="outlined" density="compact" :disabled="!isAdmin" />
    ...
    <v-dialog v-model="maintenanceDialog" max-width="760">
      <v-card><v-card-title>{{ editingMaintenance ? 'Cập nhật công việc bảo trì' : 'Tạo lịch bảo trì' }}</v-card-title><v-card-text>
        <div class="form-grid">
          ...
          <v-select v-model="maintenanceForm.assignedTo" :items="staffOptions" item-title="title" item-value="value" label="Nhân viên phụ trách" variant="outlined" density="compact" :disabled="!isAdmin" />
...
const staffOptions = computed(() => staff.value.filter((item) => item.accountStatus === 'Active').map((item) => ({ title: `${item.employeeCode || item.username} · ${item.fullName}`, value: item.username })))
...
const saveAssignment = async () => { if (!selectedIncident.value) return; if (!incidentForm.assignedTo) incidentForm.assignedTo = username; try { saving.value = true; await api.patch(`/incidents/${selectedIncident.value.id}/assign`, { assignedTo: incidentForm.assignedTo, assignedName: selectedStaff()?.fullName || localStorage.getItem('fullName'), priority: incidentForm.priority, scheduledAt: incidentForm.scheduledAt || null, dueAt: incidentForm.dueAt || null, note: incidentForm.assignmentNote }); success.value = 'Đã lưu phân công và lịch xử lý.'; await refreshSelected() } catch (err) { error.value = 'Không lưu được phân công.' } finally { saving.value = false } }
const saveProgress = async () => { if (!selectedIncident.value) return; try { saving.value = true; await api.patch(`/incidents/${selectedIncident.value.id}/status`, { status: incidentForm.status, handledBy: username, staffNote: incidentForm.staffNote, rootCause: incidentForm.rootCause, resolution: incidentForm.resolution, materialCost: incidentForm.materialCost, laborCost: incidentForm.laborCost }); success.value = 'Đã cập nhật tiến độ yêu cầu.'; await refreshSelected() } catch (err) { error.value = err.response?.data?.message || 'Không cập nhật được tiến độ.' } finally { saving.value = false } }
...
const saveMaintenance = async () => { try { saving.value = true; const assignee = staff.value.find((item) => item.username === maintenanceForm.assignedTo); if (editingMaintenance.value) await api.patch(`/maintenance/${editingMaintenance.value.id}`, { status: maintenanceForm.status, assignedTo: maintenanceForm.assignedTo, assignedName: assignee?.fullName, nextDueDate: maintenanceForm.nextDueDate, completedItems: maintenanceForm.completedItems, cost: maintenanceForm.cost, notes: maintenanceForm.notes, updatedBy: username }); else await api.post('/maintenance', { ...maintenanceForm, assignedName: assignee?.fullName, checklist: maintenanceForm.notes.split('\n').map((line) => line.trim()).filter(Boolean), notes: '' }); maintenanceDialog.value = false; success.value = 'Đã lưu lịch bảo trì.'; await loadAll() } catch (err) { error.value = err.response?.data?.message || 'Không lưu được lịch bảo trì.' } finally { saving.value = false } }
