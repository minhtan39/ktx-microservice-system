  ...
  <section class="operations-page">
    ...
    <v-dialog v-model="incidentDialog" max-width="960" scrollable>
      <v-card v-if="selectedIncident">
        ...
        <v-card-text class="incident-detail">
          <v-alert v-if="incidentError" type="error" variant="tonal" closable @click:close="incidentError = ''">
            {{ incidentError }}
          </v-alert>
          <v-alert v-if="incidentSuccess" type="success" variant="tonal" closable @click:close="incidentSuccess = ''">
            {{ incidentSuccess }}
          </v-alert>
          ...
          <section v-if="selectedIncident.aiSummary || selectedIncident.aiSuggestedAction" class="ai-panel">
            <span class="mdi mdi-auto-fix"></span>
            <div>
              <h3>Gợi ý AI</h3>
              <strong>{{ selectedIncident.aiSummary }}</strong>
              <p>{{ selectedIncident.aiSuggestedAction }}</p>
              <small v-if="selectedIncident.aiExpectedHandlingTime">Thời gian gợi ý: {{ selectedIncident.aiExpectedHandlingTime }}</small>
              <small v-if="selectedIncident.aiSafetyNote">Lưu ý: {{ selectedIncident.aiSafetyNote }}</small>
              <small>Nguồn: {{ selectedIncident.aiSource || 'Nội bộ' }}</small>
            </div>
          </section>

          <div class="detail-grid">
            <v-alert v-if="selectedIncidentFinal" type="info" variant="tonal" class="full-width">
              Yêu cầu này đã kết thúc với trạng thái "{{ statusLabel(selectedIncident.status) }}", nên không thể phân công hoặc cập nhật tiến độ lại.
            </v-alert>
            <section v-if="!selectedIncidentFinal" class="action-panel">
    ...
    <v-dialog v-model="maintenanceDialog" max-width="760">
      <v-card><v-card-title>{{ editingMaintenance ? 'Cập nhật công việc bảo trì' : 'Tạo lịch bảo trì' }}</v-card-title><v-card-text>
        <v-alert v-if="maintenanceError" type="error" variant="tonal" closable class="mb-4" @click:close="maintenanceError = ''">
          {{ maintenanceError }}
        </v-alert>
        <div class="form-grid">
...
const incidentError = ref(''), incidentSuccess = ref(''), maintenanceError = ref('')
...
const finalIncidentStatuses = ['confirmed', 'rejected', 'cancelled']
const isFinalIncident = (item) => finalIncidentStatuses.includes(String(item?.status || '').toLowerCase())
const selectedIncidentFinal = computed(() => isFinalIncident(selectedIncident.value))
const canClaimSelected = computed(() => !selectedIncidentFinal.value && (isAdmin || !selectedIncident.value?.assignedTo || selectedIncident.value?.assignedTo === username))
const canUpdateSelected = computed(() => !selectedIncidentFinal.value && (isAdmin || selectedIncident.value?.assignedTo === username))
...
const openIncident = (item) => { selectedIncident.value = item; incidentError.value = ''; incidentSuccess.value = ''; Object.assign(incidentForm, { assignedTo: item.assignedTo || (isAdmin ? '' : username), priority: item.priority || 'normal', scheduledAt: toLocalInput(item.scheduledAt), dueAt: toLocalInput(item.dueAt), assignmentNote: '', status: ['new', 'assigned'].includes(item.status) ? 'processing' : item.status, staffNote: item.staffNote || '', rootCause: item.rootCause || '', resolution: item.resolution || '', materialCost: item.materialCost || 0, laborCost: item.laborCost || 0 }); incidentDialog.value = true }
const selectedStaff = () => staff.value.find((item) => item.username === incidentForm.assignedTo)
const saveAssignment = async () => { if (!selectedIncident.value) return; if (selectedIncidentFinal.value) { incidentError.value = 'Yêu cầu đã kết thúc nên không thể phân công hoặc cập nhật lại.'; return } if (!incidentForm.assignedTo) incidentForm.assignedTo = username; try { saving.value = true; incidentError.value = ''; incidentSuccess.value = ''; const response = await api.patch(`/incidents/${selectedIncident.value.id}/assign`, { assignedTo: incidentForm.assignedTo, assignedName: selectedStaff()?.fullName || localStorage.getItem('fullName'), priority: incidentForm.priority, scheduledAt: incidentForm.scheduledAt || null, dueAt: incidentForm.dueAt || null, note: incidentForm.assignmentNote }); incidentSuccess.value = response.data?.warning || 'Đã lưu phân công và đồng bộ trạng thái phòng nếu xác định được phòng.'; await refreshSelected() } catch (err) { incidentError.value = err.response?.data?.message || 'Không lưu được phân công.' } finally { saving.value = false } }
const saveProgress = async () => { if (!selectedIncident.value) return; if (selectedIncidentFinal.value) { incidentError.value = 'Yêu cầu đã kết thúc nên không thể cập nhật tiến độ.'; return } try { saving.value = true; incidentError.value = ''; incidentSuccess.value = ''; const response = await api.patch(`/incidents/${selectedIncident.value.id}/status`, { status: incidentForm.status, handledBy: username, staffNote: incidentForm.staffNote, rootCause: incidentForm.rootCause, resolution: incidentForm.resolution, materialCost: incidentForm.materialCost, laborCost: incidentForm.laborCost }); incidentSuccess.value = response.data?.warning || (incidentForm.status === 'completed' ? 'Đã báo sửa xong. Sinh viên sẽ xác nhận kết quả trên cổng sinh viên.' : 'Đã cập nhật tiến độ yêu cầu và trạng thái phòng.'); await refreshSelected() } catch (err) { incidentError.value = err.response?.data?.message || 'Không cập nhật được tiến độ.' } finally { saving.value = false } }
...
const openCreateMaintenance = () => { editingMaintenance.value = null; maintenanceError.value = ''; Object.assign(maintenanceForm, blankMaintenance()); maintenanceDialog.value = true }
const openMaintenance = (item) => { editingMaintenance.value = item; maintenanceError.value = ''; Object.assign(maintenanceForm, { ...blankMaintenance(), ...item, nextDueDate: String(item.nextDueDate).slice(0, 10), notes: item.notes || '', checklist: [...(item.checklist || [])], completedItems: [...(item.completedItems || [])] }); maintenanceDialog.value = true }
const saveMaintenance = async () => { try { saving.value = true; maintenanceError.value = ''; const assignee = staff.value.find((item) => item.username === maintenanceForm.assignedTo); const response = editingMaintenance.value ? await api.patch(`/maintenance/${editingMaintenance.value.id}`, { status: maintenanceForm.status, assignedTo: maintenanceForm.assignedTo, assignedName: assignee?.fullName, nextDueDate: maintenanceForm.nextDueDate, completedItems: maintenanceForm.completedItems, cost: maintenanceForm.cost, notes: maintenanceForm.notes, updatedBy: username }) : await api.post('/maintenance', { ...maintenanceForm, assignedName: assignee?.fullName, checklist: maintenanceForm.notes.split('\n').map((line) => line.trim()).filter(Boolean), notes: '' }); maintenanceDialog.value = false; success.value = response.data?.warning || 'Đã lưu lịch bảo trì.'; await loadAll() } catch (err) { maintenanceError.value = err.response?.data?.message || 'Không lưu được lịch bảo trì.' } finally { saving.value = false } }
